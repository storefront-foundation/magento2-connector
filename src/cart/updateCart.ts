 ...
 * updateCart handler
 */
export default async function updateCart(item, quantity, req, res): Promise<CartResponse> {
  const cartId =
  ...
  const responseData = await fetchUC({ token, cartId, cartItemId, quantity })
